...
Given a risk neutral outlook, Policy A leads to an expected present value of climate damages up to 2100 of `r round(0.5*policy_compare_f(warming_df$damages_baseline) + 0.5*policy_compare_f(warming_df$damages_baseline_1.5), 2)`. Policy B has an expected present value of `r round(policy_compare_f(warming_df$damages_baseline_b), 2)`.

 - CHANGE IN-LINE REFERENCES ABOVE TO BILLIONS OF USD RATHER THAN SCIENTIFIC NOTATION
 - NEXT STEP IS GRAPHING TOTAL POLICY COSTS AGAINST X WHERE X IS COST OF POLICY B
